...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='claw-code — Claude-compatible interactive assistant')
    subparsers = parser.add_subparsers(dest='command', required=False)

    # ── interactive chat ──────────────────────────────────────────
    chat_parser = subparsers.add_parser('chat', help='start an interactive agentic session with Claude (default)')
    chat_parser.add_argument('--model', default='claude-sonnet-4-6', help='Anthropic model ID')
    chat_parser.add_argument('--system', default=None, help='override system prompt')
    chat_parser.add_argument('--yolo', action='store_true', help='auto-approve all tool calls without prompting')
    chat_parser.add_argument('--cwd', default=None, help='working directory for the agent')
    chat_parser.add_argument('-p', '--print', dest='prompt', default=None,
                             help='headless mode: run this prompt and exit (no REPL)')
...
def main(argv: list[str] | None = None) -> int:
    ...
    # default: no subcommand → start interactive chat
    if args.command is None or args.command == 'chat':
        ...
        run_chat(model=model, system=system, yolo=yolo, cwd=cwd,
                 print_mode=bool(prompt), prompt=prompt, resume=resume)
        return 0
